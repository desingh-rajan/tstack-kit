...
Deno.test("createProject - should add entry to KV store with correct metadata", async () => {
  const tempDir = await createTempDir();
  try {
    await createProject({
    ...
    assertEquals(metadata!.name, "kv-test-shop", "Should store original name");
...
Deno.test("createProject - should not duplicate suffix if already present", async () => {
  const tempDir = await createTempDir();
  try {
...
Deno.test("Status tracking - should track status correctly and prevent duplicate in test mode", async () => {
  const tempDir = await createTempDir();
  ...
  try {
    ...
    assertEquals(firstProject?.status, "created", "Status should be 'created'");
...
Deno.test("Status tracking - should allow recreation with forceOverwrite", async () => {
  const tempDir = await createTempDir();
...
Deno.test("Status tracking - should handle manually deleted folder", async () => {
  const tempDir = await createTempDir();
