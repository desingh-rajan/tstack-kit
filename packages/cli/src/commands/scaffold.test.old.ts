...
Deno.test(
  "scaffoldEntity - generates all 8 files by default (includes admin and tests)",
  async () => {
...
Deno.test(
  "scaffoldEntity - skipAdmin flag generates only 6 files (no admin)",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - uses correct folder naming (snake_plural)",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - uses correct file naming (kebab-singular)",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
      ...
      assertEquals(
        await fileExists(join(basePath, "blog-post.model.ts")),
        true,
      );
...
Deno.test(
  "scaffoldEntity - model file contains correct table name",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - route file contains correct endpoint paths",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - without force flag, entity dir exists check works",
  async () => {
    const tempDir = await createTempDir();
    try {
      // Create entity first time
...
Deno.test(
  "scaffoldEntity - handles irregular pluralization (person)",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - handles complex naming (site_settings)",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test("scaffoldEntity - generates valid file structure", async () => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      assertEquals(content.length > 0, true, `${file} should have content`);
    }
  } finally {
    await cleanupTempDir(tempDir);
  }
...
Deno.test(
  "scaffoldEntity - admin route has correct entity references",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - skipTests flag skips test file generation",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - default generates routes with auth middleware",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - default generates DTOs with Zod validation",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - DTOs always use Zod validation (base pattern)",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
...
Deno.test(
  "scaffoldEntity - controller extends BaseController pattern",
  async () => {
    const tempDir = await createTempDir();
    try {
      await scaffoldEntity({
