...
import type { ProductVariant } from "@/entities/product-variants/product-variant.types.ts";
...
export default define.page<typeof handler>(function ProductVariantsListPage({
  data,
}) {
  const { items, config, error, errorStatus } = data;
  const response = items as ListResponse<ProductVariant>;
